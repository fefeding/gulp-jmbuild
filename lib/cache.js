...
if(!fs.existsSync(file_cache_dir)) fs.mkdirSync(file_cache_dir, '0777');
...
exports.saveFileList = function(info) {
    var saveInfo = Object.assign({},info)
    for (var key in saveInfo) {
        if(saveInfo[key] && saveInfo[key].isHasModify){ // 删除是否修改的标记
            delete info[key].isHasModify
        }
    }
