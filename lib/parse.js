...
exports.parse =  function(options) {
    ...
    var stream = through.obj(function (file, enc, cb) {
        ...
        if (!file) {
            ...
            if(options.type == 'js') {
                ...
                var size = options.md5size || 8;
                //截取指定长度的md5码
                if(size > 0 && size < md5Hash.length) md5Hash = md5Hash.slice(0, size);
                file.md5 = md5Hash;
                var cacheInfo = cache.getInfo(file.path);
                // 是直接将文件拼接 or md5为false 则不可以忽略
                var isIgnoreCache = options.config.concat || !options.config.md5;
                var isNotDepenHtml = (cacheInfo && cacheInfo.isNotDepenHtml); // 不依赖html，即通过 __getWidgetHtml 拉取
                var isHasModify = !(cacheInfo && cacheInfo.md5 === md5Hash);
                
                if(!isHasModify && !isIgnoreCache && isNotDepenHtml){
                    // 命中缓存 且 本次未更改， 同时不依赖HTML(即无影响文件的内联依赖) 则直接跳过构建
                }else{
                    parseJS.call(this, file, options);
                    this.push(file);
                }
            }else {
                if(options.type == 'html') parseHtml.call(this, file, options);
	...
	function parseJS(file, options) {
        ...
        content = inlineJSWidgetHtml(content, options);
        ...
        var info = {
            ...
            "id": astModule.id,
            "md5": file.md5,
            "isNotDepenHtml": options.isNotDepenHtml || false
        };
        cache.setCache(key, info);
	}

    //内联JS中的组件html
    function inlineJSWidgetHtml(content, options) {
        var reg = /__getWidgetHtml\s*\(\s*([^\)\<]+?)\s*,/ig;
        options.isNotDepenHtml = true; // 标识不依赖html
        content = content.replace(reg, function(s, p) {
            var jp = p.trim().replace(/(^['"]*)|(['"]*$)/g, '');
            if(jp){
                options.isNotDepenHtml = true;
