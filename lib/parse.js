...
exports.parse =  function(options) {
    ...
    function replacePkgAndUri(content, options, buildInfo) {
        ...
        content = content.replace(reg, function(s, m, p) {
            ...
            if(jp){
                //处理md5码等后缀
                s = replaceUrlHandler(jp, options, buildInfo, destMaps);
            }
        ...
        content = content.replace(reg, function(s, p, i) {  
            //处理md5码等后缀
            var newp = replaceUrlHandler(p, options, buildInfo, destMaps);
        ...
        content = content.replace(reg, function(s, p, i) {  
            //处理md5码等后缀
            newp = replaceUrlHandler(p, options, buildInfo, destMaps);
            gutil.log(gutil.colors.blue('replace:'), gutil.colors.green(newp));
            return s.replace(p, newp);
        });
    ...
    function replaceUrlHandler(p, options, buildInfo, destMaps) {
        //去除?之后面路径
        if(p.indexOf('?') > -1) p = p.substr(0, p.indexOf('?'));
        
        var ext = path.extname(p);
        var dest = destMaps[ext] || options.destPath;
        var fpath = path.join(dest, p);
        if(!buildInfo[fpath]) {
            fpath = path.join(options.destPath, p);
        }
        //如果有md5则，合到路径中
        var info = buildInfo[fpath];

        if(info && info.path) {
            p =  path.dirname(p) + '/' + path.basename(info.path);
        }
    ...
    function replaceCSSUrl(content, options, buildInfo) {
        ...
        return content.replace(reg, function(s, p, i) {
                ...
                //相对于根发布路径
                if(!info && options.destPath) info = buildInfo[path.join(options.destPath, p)];
                
                var url = p;
                //如果有md5则，合到路径中
                if(info && info.path) {
                    url = path.dirname(p) + '/' + path.basename(info.path);
                }
                //替换映射部分
                url = replaceUrlMap(url, options.urlMaps);    
                s = s.replace(p, url);
